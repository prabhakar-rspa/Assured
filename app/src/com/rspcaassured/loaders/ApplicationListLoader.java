...
/**
 * A simple AsyncTaskLoader to load a list of Salesforce applications.
 *
 * @author shahedmiah
 */
public class ApplicationListLoader extends AsyncTaskLoader<List<ApplicationObject>> {

    public static final String APPLICATION_SOUP = "applications";
    public static final String LOAD_COMPLETE_INTENT_ACTION = "com.rspcaassured.loaders.APPLICATION_LOAD_COMPLETE";
    private static final String TAG = "ApplicationListLoader";
    private static final String SYNC_DOWN_NAME = "syncDownApplications";
    private static final String SYNC_UP_NAME = "syncUpApplications";
     ...
     */
    public ApplicationListLoader(Context context, UserAccount account) {
        super(context);
    ...
    @Override
    public List<ApplicationObject> loadInBackground() {
        Log.d("test", "ApplicationListLoader => loadInBackground");
        if (!smartStore.hasSoup(APPLICATION_SOUP)) {
            return null;
        }
        final QuerySpec querySpec = QuerySpec.buildAllQuerySpec(APPLICATION_SOUP,
                ApplicationObject.NAME, QuerySpec.Order.ascending, LIMIT);
        JSONArray results;
    ...
    public synchronized void syncUp() {
        try {
            ...
            Log.d(TAG,"Sync Status for Application Sync Up: " + ss.getStatus().toString());
    ...
    public synchronized void syncDown() {
        try {
            ...
            Log.d(TAG,"Sync Status for Application Sync Down: " + ss.getStatus().toString());
            if(ss.getStatus() != SyncState.Status.RUNNING){
